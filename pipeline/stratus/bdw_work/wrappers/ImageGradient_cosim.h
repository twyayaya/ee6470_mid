...
#ifndef _IMAGEGRADIENT_COSIM_INCLUDED_
#define _IMAGEGRADIENT_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct ImageGradient_cosim : public ncsc_foreign_module
#else
SC_MODULE(ImageGradient_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module ImageGradient is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of ImageGradient in SystemC\n", 
					scName );
				return;
			} else {
				s << "ImageGradient0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module ImageGradient are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		int i_grey_vld_is_clock = esc_link_clockgen( &i_grey_vld, sc_time( 0, SC_NS ), module_path, sim_domain, "i_grey_vld" );
		...
		esc_link_signals( &i_grey_busy, module_path, sim_domain, true );
		esc_link_signals( &i_grey_vld, module_path, sim_domain, true, inputDelay );
		esc_link_signals( &i_grey_data, module_path, sim_domain, true, inputDelay );
		...
		if ( i_clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "ImageGradient", "i_clk", scName ) ) {
			esc_report_error( esc_error, "The port 'i_clk' on module 'ImageGradient'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_rst_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "ImageGradient", "i_rst", scName ) ) {
			esc_report_error( esc_error, "The port 'i_rst' on module 'ImageGradient'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_grey_vld_is_clock )
		{
			sc_signal_in_if< bool > *clock_inif = i_grey_vld[0];
			sc_clock *clock_module = dynamic_cast<sc_clock*>(clock_inif);
			esc_hub_register_clock( clock_module, esc_alledge, 0, module_path, sim_domain, i_grey_vld.basename() );
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "ImageGradient", "vld", scName ) ) {
			esc_report_error( esc_error, "The port 'vld' on module 'ImageGradient'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( o_result_busy_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "ImageGradient", "busy", scName ) ) {
			esc_report_error( esc_error, "The port 'busy' on module 'ImageGradient'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module ImageGradient must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "ImageGradient_nc_cosim"; } 
    ImageGradient_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	ImageGradient_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" ImageGradient") ) )
		: sc_module(in_name)
#endif
		  ,i_clk("i_clk")
		  	,i_rst("i_rst")
...
int ImageGradient_cosim::numLinked = 0;
ImageGradient_cosim::StringPair ImageGradient_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
