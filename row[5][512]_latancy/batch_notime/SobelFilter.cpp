...
void SobelFilter::do_filter()
{
  ...
  int R, G, B;
	//
	sc_uint<32> Gray;
	sc_uint<8> from_tb_r[5][512];
	sc_uint<8> from_tb_g[5][512];
	sc_uint<8> from_tb_b[5][512];
	int x, y;
 //int c = 0;
	//
	{
	...
	while (true)
	{
		...
		for (x = 0; x < 512; x++)
		{
			...
			rgb = i_rgb.read();
#endif //2
			from_tb_r[4][x] = rgb.range(7, 0);
			from_tb_g[4][x] = rgb.range(15, 8);
			from_tb_b[4][x] = rgb.range(23, 16);
      ...
      //cout<<"c = "<<c<<endl;
		...
		for (int x = 0; x < 512; x++)
		{
			...
			from_tb_r[2][x] = from_tb_r[3][x];
			from_tb_g[2][x] = from_tb_g[3][x];
			from_tb_b[2][x] = from_tb_b[3][x];
			from_tb_r[3][x] = from_tb_r[4][x];
			from_tb_g[3][x] = from_tb_g[4][x];
			from_tb_b[3][x] = from_tb_b[4][x];      
      from_tb_r[4][x] = from_tb_g[4][x] = from_tb_b[4][x] = 0;
		}
	}
}
